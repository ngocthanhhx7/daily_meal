...
- Local image uploads via Multer into `server/uploads`.
- Gemini analysis through `@google/genai`.
- Development fallback calorie analysis is returned when `GEMINI_API_KEY` is empty.
...
3. Create a free post with up to 3 images.
4. Toggle Premium in Profile.
5. Create a premium post with a VIP sticker.
