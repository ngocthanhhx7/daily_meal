...
describe("getPwaEnvironment", () => {
  ...
  it("detects iPhone/iPad Safari in Desktop Mode outside standalone", () => {
    expect(getPwaEnvironment({ platform: "web", userAgent: iphoneSafariDesktop, standalone: false, maxTouchPoints: 5 })).toMatchObject({
      isIos: true,
      isSafari: true,
      isInstallCapableIosSafari: true
    });
  ...
  it("gates iOS Safari in Desktop Mode when not standalone", () => {
    expect(shouldShowIosInstallGate({ platform: "web", userAgent: iphoneSafariDesktop, standalone: false, maxTouchPoints: 5 })).toBe(true);
  });
});
